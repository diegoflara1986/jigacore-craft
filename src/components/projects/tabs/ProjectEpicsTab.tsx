...
export function ProjectEpicsTab({ projectId, isArchived = false }: { projectId: string; isArchived?: boolean }) {
  ...
  const [form, setForm] = useState<EpicFormData>({ title: "", description: "", color: EPIC_COLORS[0], start_date: undefined, end_date: undefined });

  const openCreate = () => { setEditingEpic(null); setForm({ title: "", description: "", color: EPIC_COLORS[0], start_date: undefined, end_date: undefined }); setModalOpen(true); };
  const openEdit = (e: EpicWithProgress) => { setEditingEpic(e); setForm({ title: e.title, description: e.description || "", color: e.color || EPIC_COLORS[0], start_date: e.start_date ? new Date(e.start_date) : undefined, end_date: e.end_date ? new Date(e.end_date) : undefined }); setModalOpen(true); };

  const handleSave = async () => {
    if (!form.title.trim()) return;
    const payload = { title: form.title, description: form.description || null, color: form.color, start_date: form.start_date ? format(form.start_date, "yyyy-MM-dd") : null, end_date: form.end_date ? format(form.end_date, "yyyy-MM-dd") : null };
    if (editingEpic) { await updateEpic.mutateAsync({ id: editingEpic.id, ...payload }); } else { await createEpic.mutateAsync({ project_id: projectId, ...payload }); }
    setModalOpen(false);
  };
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2"><Label>Nombre *</Label><Input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="Nombre de la épica" /></div>
            <div className="space-y-2"><Label>Descripción</Label><Textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} rows={3} /></div>
            <div className="space-y-2"><Label>Color</Label>
              <div className="flex gap-2 flex-wrap">
                {EPIC_COLORS.map((c) => (
                  <button key={c} onClick={() => setForm((p) => ({ ...p, color: c }))} className={cn("h-7 w-7 rounded-full border-2 transition-all", form.color === c ? "border-foreground scale-110" : "border-transparent")} style={{ backgroundColor: c }} />
                ))}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label>Fecha inicio</Label>
                <Popover><PopoverTrigger asChild><Button variant="outline" className={cn("w-full justify-start text-left font-normal", !form.start_date && "text-muted-foreground")}><CalendarIcon className="h-4 w-4 mr-2" />{form.start_date ? format(form.start_date, "dd/MM/yyyy") : "Seleccionar"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={form.start_date} onSelect={(d) => setForm((p) => ({ ...p, start_date: d }))} className="p-3 pointer-events-auto" /></PopoverContent></Popover>
              </div>
              <div className="space-y-2"><Label>Fecha fin</Label>
                <Popover><PopoverTrigger asChild><Button variant="outline" className={cn("w-full justify-start text-left font-normal", !form.end_date && "text-muted-foreground")}><CalendarIcon className="h-4 w-4 mr-2" />{form.end_date ? format(form.end_date, "dd/MM/yyyy") : "Seleccionar"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={form.end_date} onSelect={(d) => setForm((p) => ({ ...p, end_date: d }))} className="p-3 pointer-events-auto" /></PopoverContent></Popover>
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setModalOpen(false)}>Cancelar</Button>
            <Button onClick={handleSave} disabled={!form.title.trim() || createEpic.isPending || updateEpic.isPending}>{editingEpic ? "Guardar" : "Crear"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
