...
/**
 * Duplicates a FOR-SGSI-001 record: creates a new sig_request (status=borrador)
 * and copies the form payload. New code is generated by the DB trigger.
 */
export function useDuplicateSigForm001() {
  const queryClient = useQueryClient();
  const { profile } = useAuth();
  const workspaceId = profile?.workspace_id ?? null;
  const userId = profile?.id ?? null;
...
/**
 * Deletes a FOR-SGSI-001 record by removing its sig_request
 * (sig_form_001 cascades via request_id FK).
 */
export function useDeleteSigForm001() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (params: { id: string; requestId: string }) => {
